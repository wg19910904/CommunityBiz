...
@implementation JHGroupOrderEvaluateCell
{
...
-(void)setModel:(JHGroupDetailModel *)model{
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.face]] placeholderImage:[UIImage imageNamed:@"evaluateHead"]];
    ...
    if (label_evaluateTime == nil) {
        ...
        label_evaluateTime.text = [HZQChangeDateLine dateLineExchangeWithTime:model.evaluate_time];
    ...
    if (view_replay == nil && model.type == 3) {
    ...
    if (replayImageView == nil && model.type == 3) {
    ...
    if (label_reply == nil&& model.type == 3) {
