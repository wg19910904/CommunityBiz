...
#define kCalendarBasicColor Green_Color
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDate *selected_date = [dateFormatter dateFromString:self.current_date];
...
//             标识今天
            if ((monthInfo.month == [selected_date dateMonth]) && (monthInfo.year == [selected_date dateYear])) {
                if (indexPath.row == [selected_date dateDay] + firstWeekday - 1) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil) {
        ...
        self.current_date = [NSString stringWithFormat:@"%ld-%ld-%ld",year, month, day];
        // 刷新数据
        [_collectionViewM reloadData];
        [_collectionViewL reloadData];
        [_collectionViewR reloadData];
