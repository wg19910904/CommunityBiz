...
-(void)initData{
    self.printerData = [[NSMutableData alloc]init];
    //初始化打印机
    Byte initBytes[] = {0x1B,0x40};
    [self.printerData appendBytes:initBytes length:sizeof(initBytes)];
    //设置行间距
    Byte lineSpace[] = {0x1B,0x32};
    [self.printerData appendBytes:lineSpace length:sizeof(lineSpace)];
    //设置字体
    Byte fontBytes[] = {0x1B,0x4D,0x00};
    [self.printerData appendBytes:fontBytes length:sizeof(fontBytes)];
}
#pragma mark - -------------基本操作----------------
/**
 *  换行
 ...
 */
- (void)setAlignment:(HZQTextAlignment)alignment
{
 ...
 *  @param fontSize 字号
 */
- (void)setFontSize:(HZQFontSize)fontSize
{
...
#pragma mark  文字
- (void)appendText:(NSString *)text alignment:(HZQTextAlignment)alignment
{
    [self appendText:text alignment:alignment fontSize:HZQFontSizeTitleSmalle];
}

- (void)appendText:(NSString *)text alignment:(HZQTextAlignment)alignment fontSize:(HZQFontSize)fontSize
{
    // 1.文字对齐方式
    ...
    // 4.换行
    [self appendNewLine];
//    if (fontSize != HZQFontSizeTitleSmalle) {
//        [self appendNewLine];
//    }
}
...
{
    [self appendTitle:title value:value fontSize:HZQFontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value fontSize:(HZQFontSize)fontSize
{
    // 1.设置对齐方式
    [self setAlignment:HZQTextAlignmentLeft];
    // 2.设置字号
    [self setFontSize:fontSize];
    // 3.设置标题内容
    [self setText:title];
...
{
    [self appendTitle:title value:value valueOffset:offset fontSize:HZQFontSizeTitleSmalle];
}

- (void)appendTitle:(NSString *)title value:(NSString *)value valueOffset:(NSInteger)offset fontSize:(HZQFontSize)fontSize
{
    // 1.设置对齐方式
    [self setAlignment:HZQTextAlignmentLeft];
    // 2.设置字号
...
{
    [self setAlignment:HZQTextAlignmentLeft];
    [self setFontSize:HZQFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
        offset = 10;
    }
...
{
    // 1.设置分割线居中
    [self setAlignment:HZQTextAlignmentCenter];
    // 2.设置字号
    [self setFontSize:HZQFontSizeTitleSmalle];
    // 3.添加分割线
    NSString *line = @"- - - - - - - - - - - - - - - -";
...
- (void)appendSeperatorLineWithTitle{
    // 1.设置分割线居中
    [self setAlignment:HZQTextAlignmentCenter];
    // 2.设置字号
    [self setFontSize:HZQFontSizeTitleSmalle];
    // 3.添加分割线
    NSString *line =  NSLocalizedString(@"- - - - - - 其他费用 - - - - - -", NSStringFromClass([self class]));
    NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
    NSData *data = [line dataUsingEncoding:enc];
    [_printerData appendData:data];
    // 4.换行
...
- (void)appendFooter:(NSString *)footerInfo
{
    ...
    if (!footerInfo) {
        footerInfo =  NSLocalizedString(@"谢谢惠顾，欢迎下次光临！", NSStringFromClass([self class]));
    }
    [self appendText:footerInfo alignment:HZQTextAlignmentCenter];
}
...
#pragma mark 图片
- (void)appendImage:(UIImage *)image alignment:(HZQTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
    if (!image) {
        return;
    }
...
{
    [self appendBarCodeWithInfo:info alignment:HZQTextAlignmentCenter maxWidth:300];
}

- (void)appendBarCodeWithInfo:(NSString *)info alignment:(HZQTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
...
{
    [self appendQRCodeWithInfo:info size:size alignment:HZQTextAlignmentCenter];
}

- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size alignment:(HZQTextAlignment)alignment
{
    [self setAlignment:alignment];
...
{
    [self appendQRCodeWithInfo:info centerImage:nil alignment:HZQTextAlignmentCenter maxWidth:250];
}

- (void)appendQRCodeWithInfo:(NSString *)info centerImage:(UIImage *)centerImage alignment:(HZQTextAlignment)alignment maxWidth:(CGFloat )maxWidth
{
