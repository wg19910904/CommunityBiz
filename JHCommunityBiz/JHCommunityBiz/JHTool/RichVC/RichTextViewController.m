...
- (void)viewDidLoad {
    ...
    if (self.htmlString) {
        ...
        self.textView.attributedText = [NSMutableAttributedString toAttributedString:self.htmlString];
...
- (IBAction)fontClick:(UIButton *)sender {
    ...
    if (sender.selected) {
        self.font=25.f;
        [sender setTitle:[NSString stringWithFormat: NSLocalizedString(@"字体 %.f", NSStringFromClass([self class])),self.font] forState:UIControlStateSelected];
    }
    else
    {
        self.font=DefaultFont;
        [sender setTitle:[NSString stringWithFormat: NSLocalizedString(@"字体 %.f", NSStringFromClass([self class])),self.font] forState:UIControlStateNormal];
    }
...
- (IBAction)imageClick:(UIButton *)sender {
    ...
    __weak typeof(self) weakSelf=self;
    UIAlertController * alertVC=[UIAlertController alertControllerWithTitle: NSLocalizedString(@"选择照片", NSStringFromClass([self class])) message:@"" preferredStyle:UIAlertControllerStyleAlert];
    [alertVC addAction:[UIAlertAction actionWithTitle: NSLocalizedString(@"确定", NSStringFromClass([self class])) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakSelf selectedImage];
    }]];
    [alertVC addAction:[UIAlertAction actionWithTitle: NSLocalizedString(@"取消", NSStringFromClass([self class])) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
    }]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
   [self appenReturn];
    ...
    if (self.textView.text.length > 1) {
        ...
        self.textView.text = [self.textView.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];//去掉首尾的空白字符和换行字符
    }
