...
-(void)showSecurityViewWithBlock:(void(^)(NSString * result,NSString * code))myBlock{
    ...
    label.text = NSLocalizedString(@"请填写图形验证码", NSStringFromClass([self class]));
    ...
    myTextField.placeholder = NSLocalizedString(@"请输入图中内容", NSStringFromClass([self class]));
    ...
    [btn_true setTitle: NSLocalizedString(@"确定", NSStringFromClass([self class])) forState:UIControlStateNormal];
    ...
    [btn_cancel setTitle: NSLocalizedString(@"取消", NSStringFromClass([self class])) forState:UIControlStateNormal];
    ...
    label_error.text =  NSLocalizedString(@"输入有误,请重新输入", NSStringFromClass([self class]));
...
-(void)clickToUpData{
    ...
    [HttpTool postWithAPI:@"magic/verify" withParams:dic success:^(id json) {
        ...
        if(json){
            ...
            [self  showAlertViewWithTitle: NSLocalizedString(@"服务器繁忙,请稍后再试", NSStringFromClass([self class]))];
        }
...
-(void)clickToTrue{
    ...
    [HttpTool postWithAPI:@"magic/sendsms" withParams:dic success:^(id json) {
        ...
        if ([json[@"error"] isEqualToString:@"0"]) {
            self.block( NSLocalizedString(@"正确", NSStringFromClass([self class])),nil);
        }else{
            [self clickToUpData];
            label_error.hidden = NO;
            myTextField.text = @"";
...
-(void)clickToCancel{
    NSLog(@"点击了取消");
    [self endEditing:YES];
    self.block( NSLocalizedString(@"取消", NSStringFromClass([self class])),nil);
}
...
- (void)showAlertViewWithTitle:(NSString *)title{
    UIAlertController * alertViewController = [UIAlertController  alertControllerWithTitle: NSLocalizedString(@"温馨提示", NSStringFromClass([self class])) message:title preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle: NSLocalizedString(@"知道了", NSStringFromClass([self class])) style:UIAlertActionStyleCancel handler:nil];
