...
- (void)addButton
{
    ...
    for (int i = 0; i< 3; i++) {
        ...
        temBtn.titleLabel.font = [UIFont systemFontOfSize:15];
        [temBtn setImage:[UIImage imageNamed:@"Deliveryarrow_down"] forState:UIControlStateNormal];
        //设置选中状态下 button的字体颜色 大小及图片等
        [temBtn setTitleColor:THEME_COLOR forState:UIControlStateSelected];
        [temBtn setImage:[UIImage imageNamed:@"Deliveryarrow_up"] forState:UIControlStateSelected];
        temBtn.tag = i + 1;
        
        if (i == 2) {
            [temBtn setImage:[UIImage imageNamed:@"Deliveryarrow_down_up"] forState:UIControlStateNormal];
            [temBtn setImage:[UIImage imageNamed:@"Deliveryarrow_up_down"] forState:UIControlStateSelected];
        }
