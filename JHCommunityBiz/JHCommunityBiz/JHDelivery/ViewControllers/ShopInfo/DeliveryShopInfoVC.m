...
#import "DeliveryShopInfoVC.h"
#import "DeliveryIntroCell.h"
#import "XHChoosePhoto.h"
#import <IQKeyboardManager.h>
#import "DeliveryShopInfoVM.h"
#import "JHPickerView.h"
@interface DeliveryShopInfoVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *mainTableView;
@end

@implementation DeliveryShopInfoVC
{
    //保存店铺名
        ...
        [HttpTool postWithAPI:@"biz/waimai/info"
                   withParams:@{}
                      success:^(id json) {
                          ...
                          vm = [DeliveryShopInfoVM yy_modelWithJSON:json[@"data"]];
                          dispatch_async(dispatch_get_main_queue(), ^{
                              HIDE_HUD
                              if ([json[@"error"] isEqualToString:@"0"]) {
                                  nameField.text = vm.title;
                                  NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMAGEADDRESS,
                                                                     vm.logo]];
                                  [logoIV sd_setImageWithURL:url placeholderImage:IMAGE(@"100*100")];
                                  logoData = [NSData dataWithContentsOfURL:url];
                                  cate_id = vm.cate_id;
                                  phoneField.text = vm.phone;
                                  classifyLabel.text = vm.cate_str;
                                  introCell.introTextView.text = vm.info.length>0 ?vm.info :NSLocalizedString(@"店铺简介", nil);
...
- (void)getCate
{
    [HttpTool postWithAPI:@"biz/waimai/cate/items"
               withParams:@{}
                  success:^(id json) {
                      NSLog(@"biz/waimai/cate/items--%@",json);
                      if ([json[@"error"] isEqualToString:@"0"]) {
...
- (void)clickSureBtn:(UIButton *)sender
{
    ...
    if ([introCell.introTextView.text isEqualToString:NSLocalizedString(@"店铺简介", nil)]) {
    ...
    NSDictionary *paramDic = @{@"title":nameField.text,
                               ...
                               @"cate_id":cate_id,
                               @"info":introCell.introTextView.text};
    [HttpTool postWithAPI:@"biz/waimai/update/index"
                   params:paramDic
                  dataDic:@{@"logo":logoData}
                  success:^(id json) {
                      NSLog(@"biz/waimai/update/index--%@",json);
                      HIDE_HUD
                      if ([json[@"error"] isEqualToString:@"0"]) {
                          [JHShowAlert showAlertWithMsg:NSLocalizedString(@"保存成功", nil) withBtnTitle:NSLocalizedString(@"知道了", nil) withBtnBlock:^{
