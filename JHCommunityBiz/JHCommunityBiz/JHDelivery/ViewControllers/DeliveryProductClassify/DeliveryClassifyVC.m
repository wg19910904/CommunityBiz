...
@interface DeliveryClassifyVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *mainTableView;
@property(nonatomic,retain)NSMutableArray *deleteArray;//选中需要删除的分类
@end

@implementation DeliveryClassifyVC
{
    //编辑按钮
    UIButton *editBtn;
...
- (void)initData
{
    self.navigationItem.title = NSLocalizedString(@"外送商品分类管理", nil);
...
- (void)clickAddBtn:(UIButton *)sender
{
    ...
    __block  DeliveryAddClassifyView *backView = [[DeliveryAddClassifyView alloc]
                                         initWithTitle:NSLocalizedString(@"添加一级分类", nil)
                                         withSubTitle:@""
                                         withRemindTitle:NSLocalizedString(@"请输入一级类别名称", nil)
                                         withCancelBlock:^{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.editing) {
        ...
        DeliverySubClassifyVC *vc = [[DeliverySubClassifyVC alloc] init];
...
- (void)clickCellEditBtn:(UIButton *)sender
{
   ...
   __block DeliveryModifyClassifyView *backView = [[DeliveryModifyClassifyView alloc]
                                             initWithTitle:NSLocalizedString(@"编辑一级分类", nil)
                                              withSubTitle:@""
                                           withRemindTitle:@""
                                           withCancelBlock:^{
                                            [backView removeFromSuperview];
...
- (void)clickDeleteBtn:(UIButton *)sender
{
    ...
    [JHShowAlert showAlertWithTitle:nil withMessage:NSLocalizedString(@"您确定要删除吗?\n删除的话会使得该类下的所有子类以及子类下的商品一并删除,请谨慎操作!", nil) withBtn_cancel:NSLocalizedString(@"取消", nil) withBtn_sure:NSLocalizedString(@"确定", nil) withCancelBlock:^{
        [_deleteArray removeAllObjects];
                   ...
                   withParams:@{@"cate_id":[cate_id componentsJoinedByString:@","]}
                      success:^(id json) {
                          ...
                          if ([json[@"error"] isEqualToString:@"0"]) {
                              [JHShowAlert showAlertWithMsg:NSLocalizedString(@"删除成功", nil) withBtnTitle:NSLocalizedString(@"确定", nil) withBtnBlock:^{
                                  page = 1;
