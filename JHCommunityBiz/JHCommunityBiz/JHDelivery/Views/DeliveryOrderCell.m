...
@implementation DeliveryOrderCell
{
...
- (void)setDataModel:(DeliveryOrderCellModel *)dataModel
{
    ...
    if ([dataModel.pei_type integerValue] == 4) {  //堂食单
        remindText = [NSString stringWithFormat:@"  %@",NSLocalizedString(@"堂食单", nil)];
    }
    ...
    _addressLabel.text = [dataModel.addr length] == 0 ?(dataModel.pei_type.integerValue == 3? NSLocalizedString(@"自提", nil): NSLocalizedString(@"堂食", nil)): [dataModel.addr stringByAppendingString:dataModel.house ?dataModel.house:@""];
    ...
    //添加按钮
    _giveUpBtn.frame = FRAME(WIDTH - 160, 150+height_addr + getStrHeight(_noteLabel.text, WIDTH - 30, 14)+15, 80, 30);
    [_giveUpBtn setTitle:NSLocalizedString(@"放弃接单", nil) forState:(UIControlStateNormal)];
    [_giveUpBtn setTitleColor:THEME_COLOR forState:(UIControlStateNormal)];
    _actionBtn.frame = FRAME(WIDTH - 80, 150+height_addr + getStrHeight(_noteLabel.text, WIDTH - 30, 14)+15, 80, 30);
