...
@implementation DeliveryAddClassifyView
{
    UIView *centerView;
    UILabel *titleL;
    UILabel *subL;
                ...
                withSureBlock:(void(^)())sureBlock
{
    ...
    if (self) {
        ...
        self.urlLink=@"biz/waimai/product/cate/create";
    }
...
- (void)clickSureBtn
{
    ...
    SHOW_HUD_IN_SELF
    NSMutableDictionary *paramDic = @{@"title":title}.mutableCopy;
    
    if (self.is_sub) {
        if (self.parent_id) [paramDic addEntriesFromDictionary:@{@"cate_id":_parent_id}];
        [paramDic addEntriesFromDictionary:@{@"number":sort_num}];
    }else{
        if (self.parent_id) [paramDic addEntriesFromDictionary:@{@"parent_id":_parent_id}];
        [paramDic addEntriesFromDictionary:@{@"orderby":sort_num}];
    }
