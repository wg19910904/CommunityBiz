...
#import "DSToast.h"
@interface JHShopEvaluateAllVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHShopEvaluateAllVC
- (void)viewDidLoad {
    [super viewDidLoad];
    //初始化一些数据
    [self initData];
...
-(void)postHttpWithPage:(NSString *)page{
    [HttpTool postWithAPI:@"biz/comment/items" withParams:@{@"page":page} success:^(id json) {
        NSLog(@"%@",json);
