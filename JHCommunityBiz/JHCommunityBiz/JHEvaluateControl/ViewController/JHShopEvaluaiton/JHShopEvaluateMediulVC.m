...
#import "DSToast.h"
@interface JHShopEvaluateMediulVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHShopEvaluateMediulVC
- (void)viewDidLoad {
    [super viewDidLoad];
    //初始化一些数据
    [self initData];
...
-(void)postHttpWithPage:(NSString *)page{
    [HttpTool postWithAPI:@"biz/comment/items" withParams:@{@"page":page,@"type":@"2"} success:^(id json) {
        NSLog(@"%@",json);
