...
#import "DSToast.h"
@interface JHShopEvaluatePoorVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHShopEvaluatePoorVC
- (void)viewDidLoad {
    [super viewDidLoad];
    //初始化一些数据
    [self initData];
...
-(void)postHttpWithPage:(NSString *)page{
    [HttpTool postWithAPI:@"biz/comment/items" withParams:@{@"page":page,@"type":@"3"} success:^(id json) {
        NSLog(@"%@",json);
