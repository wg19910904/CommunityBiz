...
#import "DSToast.h"
@interface JHPoorEvaluationVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHPoorEvaluationVC
- (void)viewDidLoad {
    [super viewDidLoad];
    //初始化一些数据
    [self initData];
...
-(void)postHttpWithPage:(NSString *)page{
    [HttpTool postWithAPI:@"biz/waimai/comment/comment/items" withParams:@{@"page":page,@"type":@"3"} success:^(id json) {
        NSLog(@"%@",json);
