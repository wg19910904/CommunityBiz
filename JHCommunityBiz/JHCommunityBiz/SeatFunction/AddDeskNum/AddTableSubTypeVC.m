...
@interface AddTableSubTypeVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *mainTableView;
@end

@implementation AddTableSubTypeVC
{
    //编辑按钮
    UIButton *editBtn;
    //父级类别
...
- (void)initData
{
    self.navigationItem.title =  NSLocalizedString(@"桌号管理", NSStringFromClass([self class]));
    page = 1;
...
{
    SHOW_HUD
    [HttpTool postWithAPI:@"biz/yuyue/zhuohao/items"
               withParams:@{@"cate_id":_parent_id,@"page":@(page)}
                  success:^(id json) {
                      ...
                      NSLog(@"biz/yuyue/zhuohao/items---%@",json);
                      if ([json[@"error"] isEqualToString:@"0"]) {
                          if(page==1) [dataSource removeAllObjects];
...
{
    __weak typeof(self)weakSelf = self;
    NSString *subtitle = [NSString stringWithFormat:@"%@%@", NSLocalizedString(@"桌号分类:", NSStringFromClass([self class])),self.supCateTitle];
    __block  DeliveryAddClassifyView *backView = [[DeliveryAddClassifyView alloc]
                                                  initWithTitle: NSLocalizedString(@"添加桌号", NSStringFromClass([self class]))
                                                  withSubTitle:subtitle
                                                  withRemindTitle: NSLocalizedString(@"请输入桌号名称", NSStringFromClass([self class]))
                                                  withCancelBlock:^{
    ...
    backView.is_sub=YES;
    backView.urlLink=@"biz/yuyue/zhuohao/create";
    backView.remindLabel.text= NSLocalizedString(@"此桌的就餐人数", NSStringFromClass([self class]));
    backView.numPlaceHolder= NSLocalizedString(@"最大人数", NSStringFromClass([self class]));
...
- (void)clickCellEditBtn:(UIButton *)sender
{
    ...
    __block DeliveryModifyClassifyView *backView = [[DeliveryModifyClassifyView alloc]
                                                    initWithTitle: NSLocalizedString(@"编辑桌号", NSStringFromClass([self class]))
                                                    withSubTitle:subtitle
                                                    withRemindTitle:@""
                                                    withCancelBlock:^{
                                                        [backView removeFromSuperview];
                                                    ...
                                                    withOrderBy:dataModel.number
                                                    withCate_id:dataModel.cate_id];
    
    backView.is_sub=YES;
    backView.remindLabel.text=  NSLocalizedString(@"此桌的就餐人数", NSStringFromClass([self class]));
    backView.numPlaceHolder=  NSLocalizedString(@"最大人数", NSStringFromClass([self class]));
    backView.zhuohao_id=dataModel.zhuohao_id;
    backView.urlLink=@"biz/yuyue/zhuohao/edit";
    [self.view addSubview:backView];
...

{
    ...
    [JHShowAlert showAlertWithTitle:nil withMessage: NSLocalizedString(@"您确定要删除此桌号吗?\n请谨慎操作!", NSStringFromClass([self class])) withBtn_cancel: NSLocalizedString(@"取消", NSStringFromClass([self class])) withBtn_sure: NSLocalizedString(@"确定", NSStringFromClass([self class])) withCancelBlock:^{
        [_deleteArray removeAllObjects];
        ...
        for (NSString *section in _deleteArray) {
            ...
            [cate_id addObject:model.zhuohao_id];
        }
        //删除分类
        SHOW_HUD
        [HttpTool postWithAPI:@"biz/yuyue/zhuohao/delete"
                   withParams:@{@"zhuohao_id":[cate_id componentsJoinedByString:@","]}
                      success:^(id json) {
                          HIDE_HUD
                          __weak typeof(self)weakSelf = self;
                          if ([json[@"error"] isEqualToString:@"0"]) {
                              [JHShowAlert showAlertWithMsg: NSLocalizedString(@"删除成功", NSStringFromClass([self class]))];
