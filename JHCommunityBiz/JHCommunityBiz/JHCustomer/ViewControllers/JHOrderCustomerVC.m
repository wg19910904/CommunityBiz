...
#import "JHOrderCustomerCellTwo.h"
#import "JHCustomerModel.h"
#import "JHCustomerCellNone.h"
#import <MJRefresh.h>
#import "JHCustomerVC.h"
#import "DSToast.h"
@interface JHOrderCustomerVC ()<UITableViewDataSource,UITableViewDelegate>
{
    ...
    BOOL isYes;//模拟用的
    NSInteger num;
    NSMutableArray * infoArray;//存放下单的客户的
    DSToast * toast;
    NSString * total_count;
}
@end

@implementation JHOrderCustomerVC
...
-(void)postHttpWithPage:(NSString *)page{
    [HttpTool postWithAPI:@"biz/member/order" withParams:@{@"page":page} success:^(id json) {
        NSLog(@"%@",json);
        if ([json[@"error"] isEqualToString:@"0"]) {
            ...
            for (NSDictionary * dic  in tempArray) {
                JHCustomerModel * model = [JHCustomerModel showJHCustomerModelWithDictionary:dic];
...
-(void)creatUITableView{
    ...
    [myTableView registerClass:[JHOrderCustomerCellOne class] forCellReuseIdentifier:@"cell"];
    [myTableView registerClass:[JHOrderCustomerCellTwo class] forCellReuseIdentifier:@"cell1"];
...
-(UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (!isYes) {
        if (indexPath.row == 0) {
            JHOrderCustomerCellOne * cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
            cell.num = [total_count integerValue];
            return cell;
        }else{
            JHOrderCustomerCellTwo * cell = [tableView dequeueReusableCellWithIdentifier:@"cell1" forIndexPath:indexPath];
            if (infoArray.count > 0) {
                JHCustomerModel * model = infoArray[indexPath.row - 1];
                cell.model = model;
            }
           return cell;
        }
        ...
        cell.str = NSLocalizedString(@"暂无客户", nil);
